...
function findButtonName(names, buttonId, playerIndex) {
  ...
  return (
    ...
    names.find(
      (name) =>
        name.toLowerCase().includes(playerPrefix.toLowerCase()) &&
...
function usePressedControls({ host, port, password, card, player }) {
  ...
  const sendCard = () => {
    ...
    if (!/^[0-9A-F]{16}$/.test(cardId)) {
      setError(
        "La tarjeta debe tener exactamente 16 caracteres hexadecimales.",
      );
      return;
    }
    ...
    apiRef.current
      .request("card", "insert", [playerIndex, cardId])
      .then(() => setError(""))
  ...
  const addCredit = () => {
    if (!apiRef.current?.connected) {
      const message = "Conecta con Spice2x antes de añadir créditos.";
      setError(message);
          ...
          .then((data) => {
            ...
            directionNamesRef.current = new Map(
              buttonIds.map((buttonId) => [
                buttonId,
                findButtonName(names, buttonId, playerIndex),
              ]),
            );
          })
          .catch((requestError) => setError(requestError.message));
